...
class EnergyProductController extends Controller
{
    ...
    {
        $products = Product::where('product_category_id', 2)->where('status', 1)->orderBy('price_sale')->paginate(20);
        return view('category_products', ['products' => $products, 'title' => 'Nước tăng lực & giải khát']);
    }

    public function energyProduct(Request $request)
    {
        ...
        if ($min_price && $max_price) {
            ...
            if ($sortby == 'price-desc')
                $products = Product::select('id', 'name', 'slug', 'image', 'price_sale', 'price', 'quantity')->where('product_category_id', 2)->where('status', 1)->orderBy('price_sale', 'desc');
            else if ($sortby == 'name')
                $products = Product::select('id', 'name', 'slug', 'image', 'price_sale', 'price', 'quantity')->where('product_category_id', 2)->where('status', 1)->orderBy('name');
            else if ($sortby == 'date')
                $products = Product::select('id', 'name', 'slug', 'image', 'price_sale', 'price', 'quantity')->where('product_category_id', 2)->where('status', 1)->orderBy('created_at', 'desc');
            else
